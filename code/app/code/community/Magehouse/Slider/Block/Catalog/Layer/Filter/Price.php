...
class Magehouse_Slider_Block_Catalog_Layer_Filter_Price extends Mage_Catalog_Block_Layer_Filter_Price 
{
	...
	*/
	...
	public function getPriceDisplayType(){
		...
		if($this->isTextBoxEnabled()){
			...
			$html = '<p>
						<input type="text" id="amount" readonly="readonly" style="background:none; border:none;" value="'.$this->getCurrencySymbol().$this->toFixed($this->getCurrMinPrice())." - ".$this->getCurrencySymbol().$this->toFixed($this->getCurrMaxPrice()).'" />
					</p>';	
		}
		return $html;
	}
	...
	*/
	public function setCurrentPrices(){
		...
		$this->_currMinPrice = $this->toFixed($this->getRequest()->getParam('min'));
		$this->_currMaxPrice = $this->toFixed($this->getRequest()->getParam('max'));
	}	
